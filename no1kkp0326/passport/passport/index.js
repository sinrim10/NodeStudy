...
var routes = function routes(app){
    //페이스북
	app.get(config.routes.facebookAuth, passport.authenticate('facebook')); //페이스북쪽에 인증요청
	app.get(config.routes.facebookAuthCallback, passport.authenticate('facebook',
		{successRedirect: config.routes.chat, failureRedirect: config.routes.login, failureFlash: true}));  //페이스죽에서 인증완료해준다.
    //구글
